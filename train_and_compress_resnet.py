...
def training():
    ...
    if not os.path.exists('./results/'):
        os.mkdir('./results/')
    save_history(history, './results/', 'resnet_{}'.format(args.depth))
    model.save_weights('./results/resnet_{}_weights.h5'.format(args.depth))

    dropped_weights = compression.select_best_model(model, x_test, y_test, iter=100)
    compression.save_pruned_weights(dropped_weights, 'resnet_{}_compressed_weights'.format(args.depth))


if __name__ == '__main__':
    parse = argparse.ArgumentParser()
    parse.add_argument('--data', type=str, default='c10', help='Supports c10 (CIFAR-10) and c100 (CIFAR-100)')
    parse.add_argument('--depth', type=int, default=20, help='model depth, supports 20, 38, 56')
