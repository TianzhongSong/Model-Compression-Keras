...
def inception_v3(nb_classes, is_training=True):
    ...
    # stage 1 32x32x64
    for _ in range(2):
        branch1x1 = conv2d_factory(x, 16, (1, 1), is_training=is_training)

        branch5x5 = conv2d_factory(x, 16, (1, 1), is_training=is_training)
        branch5x5 = conv2d_factory(branch5x5, 16, (5, 5), is_training=is_training)

        branch3x3 = conv2d_factory(x, 16, (1, 1), is_training=is_training)
        branch3x3 = conv2d_factory(branch3x3, 16, (3, 3), is_training=is_training)
        branch3x3 = conv2d_factory(branch3x3, 16, (3, 3), is_training=is_training)

        branch_pool = AveragePooling2D(pool_size=(2, 2), strides=(1, 1), padding='same')(x)
        branch_pool = conv2d_factory(branch_pool, 16, (1, 1), is_training=is_training)
    ...
    for _ in range(2):
        ...
        branch5x5 = conv2d_factory(branch5x5, 32, (5, 1), is_training=is_training)
        branch5x5 = conv2d_factory(branch5x5, 32, (1, 5), is_training=is_training)

        branch7x7 = conv2d_factory(x, 32, (1, 1), is_training=is_training)
        branch7x7 = conv2d_factory(branch7x7, 32, (7, 1), is_training=is_training)
        branch7x7 = conv2d_factory(branch7x7, 32, (1, 7), is_training=is_training)
        branch7x7 = conv2d_factory(branch7x7, 32, (7, 1), is_training=is_training)
        branch7x7 = conv2d_factory(branch7x7, 32, (1, 7), is_training=is_training)
    ...
    for _ in range(2):
        ...
        branch3x3_1 = conv2d_factory(x, 64, (1, 1), is_training=is_training)
        branch3x3_1 = conv2d_factory(branch3x3_1, 64, (3, 1), is_training=is_training)
        branch3x3_1 = conv2d_factory(branch3x3_1, 64, (1, 3), is_training=is_training)

        branch3x3_2 = conv2d_factory(x, 64, (1, 1), is_training=is_training)
        branch3x3_2 = conv2d_factory(branch3x3_2, 64, (3, 3), is_training=is_training)
        branch3x3_2 = conv2d_factory(branch3x3_2, 64, (3, 1), is_training=is_training)
        branch3x3_2 = conv2d_factory(branch3x3_2, 64, (1, 3), is_training=is_training)

        branch_pool = AveragePooling2D(pool_size=(2, 2), strides=(1, 1), padding='same')(x)
        branch_pool = conv2d_factory(branch_pool, 64, (1, 1), is_training=is_training)

        x = concatenate([branch1x1, branch3x3_1, branch3x3_2, branch_pool], axis=-1)
